...
def main():
    ...
    camera_movement_per_frame = camera_movement_estimator.get_camera_movement(
        frames, read_from_stub=True, stub_path="stubs/camera_movement_stubs.pkl"
    )
    ...
    #     bbox = player["bbox"]
    #     cropped_image = frames[0][
    #         int(bbox[1]) : int(bbox[3]), int(bbox[0]) : int(bbox[2])
    #     ]
    #     cv2.imwrite(f"output_videos/player_{track_id}.png", cropped_image)
    #     break
