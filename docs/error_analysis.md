...
3. **Validated Algorithmic Mitigations**:
   ...
   - Superhuman pass velocity threshold filtering ($> 45.0\text{ m/s}$ / $162\text{ km/h}$).
   - Optical flow discontinuity detection and vector reset across broadcast scene cuts ($> 25\text{ px/frame}$).
   - Possession temporal hysteresis buffer ($\ge 2$ frames) suppressing scrum assignment flicker.
   - Temporal gap-filling buffer ($\le 2$ frames) bridging short ball detection dropouts.
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **`TS_PASS_01`** | Tracking | Track-Swap False Pass | Track ID switch on same player ($\Delta d < 1.5\text{ m}, \Delta t \le 2\text{ frames}$) | 1.0 false passes | 0.0 false passes | **100.0%** |
| **`SH_VEL_02`** | Tracking | Superhuman Velocity | Bounding box teleportation across pitch ($v > 45\text{ m/s}$) | 1.0 false passes | 0.0 false passes | **100.0%** |
...
* **File**: [`src/football_cv/analytics/event_builder.py`](file:///d:/football-cv/src/football_cv/analytics/event_builder.py)
...
* **File**: [`src/football_cv/possession/events.py`](file:///d:/football-cv/src/football_cv/possession/events.py)
...
* **File**: [`src/football_cv/camera_motion/estimator.py`](file:///d:/football-cv/src/football_cv/camera_motion/estimator.py)
